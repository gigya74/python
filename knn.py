...
def scree_plot(pca, title=None):
    ...
    if title is not None:
     plt.title(title)
def calc_mat(y_true, y_predict):
    ys = zip(y_true, y_predict)
    tp, fp, tn, fn = 0, 0, 0, 0
    for t, p in ys:
        if t == p and t == 1:
            tp += 1
    ...
    index = ['True +', 'True -']
    cm = pd.DataFrame(np.array([[tp, fn], [fp, tn]]), index=index, columns=cols)
    print(cm)
...
if __name__ == '__main__':
    ...
    print(calc_mat(y_test, knn.predict(pca.transform(x_test))))
